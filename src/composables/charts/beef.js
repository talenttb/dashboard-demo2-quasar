import { ref } from 'vue'
import beefSvg from '@/assets/Beef_cuts_France.svg'
import { selfResourceApi } from 'boot/axios'
import { registerMap } from 'echarts/core'

export default function beef() {
  const optionBeefSvg = ref({})
  const loadSvg = async () => {
    let response = await selfResourceApi.get(beefSvg).catch((err) => {
      return { data: 'ggggggggggggggggggggggg' }
    })
    // console.log(response.data)
    registerMap('Beef_cuts_France', { svg: response.data })
    optionBeefSvg.value = {
      tooltip: {},
      visualMap: {
        left: 'center',
