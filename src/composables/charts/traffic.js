import { ref } from 'vue'
import planSvg from '@/assets/ksia-ext-plan-min.svg'
import { selfResourceApi } from 'boot/axios'
import { registerMap } from 'echarts/core'

export default function traffic() {
  const optionTrafficSvg = ref({})
  const loadSvg = async () => {
    let response = await selfResourceApi.get(planSvg).catch((err) => {
      return { data: 'ggggggggggggggggggggggg' }
    })
    // console.log(response.data)
    registerMap('ksia-ext-plan', { svg: response.data })
    optionTrafficSvg.value = {
      tooltip: {},
      geo: {
        map: 'ksia-ext-plan',
        roam: true,
